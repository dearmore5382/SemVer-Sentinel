...
test('release readback parser enforces exact shape', () => {
  const raw = ['REVIEWED', address, 'p-limit', '3.0.0', '3.1.0', 'MINOR', 'NON_BREAKING', 'COMPLIANT', 'COMPATIBLE_CHANGE', '{"surface_change":"ADDITIVE"}'].join('|');
...
test('method-specific authoritative readback is mandatory', () => {
  const base = { hash: '0x' + '2'.repeat(64), sender: address, contract: address, args: ['p-limit', '3.0.0', '3.1.0', 'policy'], method: 'create_release', chainId: 61999, stage: 'PENDING', detail: '', createdAt: '' };
  const current = { publisher: address, package: 'p-limit', oldVersion: '3.0.0', newVersion: '3.1.0', status: 'DRAFT', compliance: 'UNEVALUATED' };
