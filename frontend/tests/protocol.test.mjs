...
const artifact = `https://raw.githubusercontent.com/acme/signal-kit/${commit}/release.json`;
...
test('release readback parser enforces exact shape', () => {
  const raw = ['REVIEWED', address, 'github:acme/signal-kit', '1.4.2', '1.5.0', 'MINOR', artifact, commit, hash, hash, 'NON_BREAKING', 'COMPLIANT', 'COMPATIBLE_CHANGE', '{"surface_change":"ADDITIVE"}'].join('|');
...
test('method-specific authoritative readback is mandatory', () => {
  const base = { hash: '0x' + '2'.repeat(64), sender: address, contract: address, args: ['1.4.2', '1.5.0', 'policy', artifact, hash], method: 'create_release', chainId: 61999, stage: 'PENDING', detail: '', createdAt: '' };
  const current = { publisher: address, package: 'github:acme/signal-kit', oldVersion: '1.4.2', newVersion: '1.5.0', artifactUrl: artifact, expectedDigest: hash, status: 'DRAFT', compliance: 'UNEVALUATED' };
  assert.equal(verifyReleaseReadback(base, '0', current), true);
  assert.throws(() => verifyReleaseReadback(base, '0', { ...current, artifactUrl: artifact.replace('release.json', 'other.json') }));
  ...
  assert.equal(verifyReleaseReadback({ ...base, method: 'assess_release', args: ['0'] }, 'ARTIFACT_REJECTED', { ...current, status: 'REJECTED', compliance: 'ARTIFACT_REJECTED' }), true);
});
